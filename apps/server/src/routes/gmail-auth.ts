...
gmailAuth.post("/configure", async (c) => {
  ...
  const googleProjectId = body?.googleProjectId?.trim();
  const googleClientId = body?.googleClientId?.trim();
  const googleClientSecret = body?.googleClientSecret?.trim();

  if (!googleProjectId || !googleClientId || !googleClientSecret) {
    return c.json({ error: "Missing project credentials" }, 400);
  }
